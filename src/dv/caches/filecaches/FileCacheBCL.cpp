...
constexpr char FileCacheBCL::kCacheName[];

FileCacheBCL::FileCacheBCL(DV *dv_ptr, ID_type capacity, ID_type protected_mrus)
    : FileCacheLRU(dv_ptr, capacity) {
 ...
 */
void FileCacheBCL::actualPut(const std::string &key, std::unique_ptr<FileDescriptor> value) {
...
FileDescriptor *FileCacheBCL::actualGet(const std::string &key) {
    return FileCacheLRU::actualGet(key);
}

FileCacheBCL::id_cost_pair_type FileCacheBCL::findVictim() {
...
bool FileCacheBCL::isCostAware() const {
    return kCostAware;
}

bool FileCacheBCL::isPartitionAware() const {
 ...
 */
FileCacheBCL::id_cost_pair_type FileCacheBCL::findVictim_BCL_raw() {
    // run 1
...
void FileCacheBCL::depreciateAcost(dv::cost_type amount) {
