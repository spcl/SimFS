...
//
// additional options can be added, of course
...
constexpr char kMsgGetStatus[] = "S";

constexpr int kMaxBufferLen = 4096;

const string name = "DV status";
...
int main(int argc, char *argv[]) {
	...
	string get_status_message = string(kMsgGetStatus) + ":" + appid_string + ":status";
	reply = connectAndSend(address, port, get_status_message, true);
	cout << "get_status message sent to DV server @ " << address << ":" << port << endl;

	KeyValueStore status;
	status.fromString(reply);
	vector<string> keys;
	for (const auto &item : status.getStoreMap()) {
		keys.push_back(item.first);
	}
	sort(keys.begin(), keys.end());
	cout << endl << "status" << endl;
	for (const auto &key : keys) {
		cout << key << ": " << status.getString(key) << endl;
	}
	cout << endl;
}
